...
int DrawGLScene2(GLvoid)									// Here's Where We Do All The Drawing
{
	...
	glClearColor( 0.7f, 0.9f, 1, 1 );
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);	// Clear Screen And Depth Buffer
	glMatrixMode( GL_PROJECTION );
	//gluPerspective( 0.6, 1.2, 0.1, 2000 );
	glLoadMatrixf( proj );
	...
	//< start VIOSO API code
	if( pWarper && VWB_render )
		VWB_render(pWarper, (VWB_param)-1, VWB_STATEMASK_ALL);// VWB_STATEMASK_STANDARD);
	//< end VIOSO API code
...
int DrawGLScene3(GLvoid)									// Here's Where We Do All The Drawing
{
	GLfloat view[16], proj[16];
	GLfloat eye[3] = {0,0,0};
	GLfloat rot[3] = {0,0,0};
	...
	GLfloat a = 3.0f;
 	glBegin(GL_TRIANGLES);							// Start Drawing A Triangle
	glColor3f( 1.0f, 0, 0 );
	glVertex3d( -1.0f,-1.0f, -a);
	glColor3f( 0, 1.0f, 0 );
    glVertex3d(  1.0f,-1.0f, -a);
	glColor3f( 0, 0, 1.0f );
    glVertex3d(  0.0f, 1.0f, -a);
								 
	glColor3f( 0, 1.0f, 0 );	 
    glVertex3d( -a,-1.0f,  1.0);
    glVertex3d( -a,-1.0f, -1.0);
    glVertex3d( -a, 1.0f,  0.0);
    							 
	glColor3f( 0, 0, 1.0f );	 
	glVertex3d( a,-1.0f, -1.0f);
    glVertex3d( a,-1.0f,  1.0f);
    glVertex3d( a, 1.0f,  0.0f);
